...
func TestSegment_Match(t *testing.T) {
	...
	a.True(seg.Match(p)).
		Empty(p.Path).
		Equal(p.Params, []params.Param{{K: "id", V: "1"}})
	...
	a.True(seg.Match(p))
	a.Equal(p.Path, "/email").Equal(p.Params, []params.Param{{K: "id", V: "1"}})
	...
	a.True(seg.Match(p)).
		Equal(p.Path, "").
		Equal(p.Params, []params.Param{{K: "id", V: "1/2"}})
	...
	a.True(seg.Match(p)).
		Equal(p.Path, "123").
		Equal(p.Params, []params.Param{{K: "any", V: ""}})
	...
	a.True(seg.Match(p))
	a.Empty(p.Path).
		Equal(p.Params, []params.Param{{K: "any", V: "123"}})
	...
	a.True(seg.Match(p)).
		Empty(p.Path).
		Equal(p.Params, []params.Param{{K: "any", V: "12345"}})
	...
	a.True(seg.Match(p)).
		Empty(p.Path).
		Equal(p.Params, []params.Param{{K: "path", V: "/posts/author"}})
	...
	a.True(seg.Match(p))
	a.Empty(p.Path).
		Equal(p.Params, []params.Param{{K: "id", V: "123"}})

	// Named:digit Endpoint 不匹配，不会删除传入的参数
	seg, err = i.NewSegment("{id:digit}")
	a.NotError(err).NotNil(seg)
	p = &params.Params{Path: "one", Params: []params.Param{{K: "p1", V: "v1"}}}
	a.False(seg.Match(p)).
		Equal(p.Path, "one").
		Equal(p.Params, []params.Param{{K: "p1", V: "v1"}})
	...
	// Named:digit 类型不匹配
	p = &params.Params{Path: "xx/author", Params: []params.Param{}}
	a.False(seg.Match(p)).
	...
	// String 匹配
	p = &params.Params{Path: "/posts/author", Params: []params.Param{{K: "p1", V: "v1"}}}
	a.True(seg.Match(p)).
		Empty(p.Path).
		Equal(p.Params, []params.Param{{K: "p1", V: "v1"}})
